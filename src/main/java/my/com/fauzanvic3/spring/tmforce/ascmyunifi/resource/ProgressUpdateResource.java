...
//    @UnitOfWork
    ...
    @ApiResponses(value = {
        @ApiResponse(code = 200, message = "Success", response = MyUnifiProgressUpdateMainRequest.class),
        @ApiResponse(code = 400, message = "Error 400, 404, 500", response = ExceptionResponse.class)
    })
    public MyUnifiProgressUpdateMainResponse progressUpdate(@HeaderParam(TMF_REQUEST) String requestId, MyUnifiProgressUpdateMainRequest request) throws WebApplicationException{
